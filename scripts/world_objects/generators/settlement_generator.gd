...
func create_settlements(start_pos_x, start_pos_z, end_pos_x, end_pos_z, step) -> Array[Vector3]:
	...
	for x in (end_pos_x - start_pos_x) / step:
		for z in (end_pos_z - start_pos_z) / step:
			...
			var settlement_radius = add_settlement(position)
			result.append(Vector3(position.x, position.z, settlement_radius))
	return result

# Returns radius of settlement
func add_settlement(position: Vector3) -> float:
